...
public:
  Log(const char* file, const char* func, int line, xop::LogLevel  level)
	: m_file(basename(file))
	, m_func(func)
	, m_line(line)
	, m_level(level) {}
  ~Log() { log_noLevelCmp(m_level, "%s[%s::%s():%d]", m_stream.str().c_str(), m_file, m_func, m_line); }
...
//自定义
LogFunc Log::logfunc = mlog;

#define LOG() (Log(__FILE__, __FUNCTION__, __LINE__, xop::LogLevel::Info).stream())	
class Logger
{
public:
